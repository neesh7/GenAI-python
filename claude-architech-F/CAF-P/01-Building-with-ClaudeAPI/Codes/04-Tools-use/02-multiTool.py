...
def run_tools(message):
    ...
    for tool_request in tool_requests:
        try:
            ...
            tool_result_block = {
                "type": "tool_result",
                "tool_use_id": tool_request.id,  # Links result back to tool call
            ...
            tool_result_block = {
                "type": "tool_result",
                "tool_use_id": tool_request.id,  # Links error back to tool call
...
def run_conversation(messages):
    """Run the multi-turn conversation loop: get Claude response → execute tools if needed → repeat until final answer."""
    ...
    while True:
        # Step 1: Call Claude with current message history and available tools
        response = chat(
            messages,
        ...
        # Step 2: Add Claude's response to conversation history
        add_assistant_message(messages, response)

        # Step 3: Print any text content from the response
        print(text_from_message(response))

        # Step 4: Check if Claude is done or wants to use tools
        ...
        # Step 5: Execute all tools Claude requested
        tool_results = run_tools(response)

        # Step 6: Add tool results back to conversation history as user message
