...
##2. Searching
    ...
    # 3. Normalization
    # 4. Similarty search conducted in Vector DB(Cosine Similarity)
    # 5. Relevant chunks as per user query is returned

## Note: Cosine of the angle between 2 vector. if they're in the exact same direction we get cosine similary of 1.0 and if not we get -1.0.
# if perpendicular then 0.0
# cosine distance = 1 - cosine similarity
# so in cosine distance, value close to 0 means a large similarity and value away from 0 not that of a similarty

## 3. Prompt prep
    # 1. User Query + relevant chunks -> sent to AI/ LLM
...
#### Chunking
# 2. Chunk by section- Structure Based Chunking
...
# 4. Create a vector store and add each embedding to it
store = VectorIndex()

for embedding, chunk in zip(embeddings, chunks):
    store.add_vector(embedding, {"content": chunk})

# 5. Some time later, a user will ask a question. Generate an Embedding for it.
user_embedding = generate_embeddings("What did the software engineering department did do last year?")

# 6. Search the store with the embeddings, find the 2 most relevant chunks
